          ...
          {(detailedInfo || movie) && (
            <ScrollArea className="h-full max-h-[80vh]">
              <div className="h-full">
                ...
                {/* Сиквелы и приквелы - вне основного padding */}
                {detailedInfo?.sequelsAndPrequels &&
                  detailedInfo.sequelsAndPrequels.length > 0 && (
                    <div className="mt-6">
                      <h3 className="text-lg font-semibold text-foreground mb-3 px-6">
                        Сиквелы и приквелы
                      </h3>
                      <div className="flex flex-wrap justify-start gap-x-6 gap-y-8 px-6 pb-4">
                        {detailedInfo.sequelsAndPrequels.map((item) => {
                          const itemDetails = item.details;
                          // Формируем объект фильма в формате MovieCard
                          const movieData = {
                            id: itemDetails.id || itemDetails.ident,
                            ident: itemDetails.ident || itemDetails.id,
                            title: itemDetails.name,
                            poster: itemDetails.poster,
                            year: itemDetails.released,
                            rating:
                              itemDetails.rating_imdb || itemDetails.rating_kp,
                            age: itemDetails.age,
                ...
                {/* Похожие фильмы - вне основного padding */}
                {detailedInfo?.similars && detailedInfo.similars.length > 0 && (
                  <div className="mt-6">
                    <h3 className="text-lg font-semibold text-foreground mb-3 px-6">
                      Похожие фильмы
                    </h3>
                    <div className="flex flex-wrap justify-start gap-x-6 gap-y-8 px-6 pb-6">
                      {detailedInfo.similars.map((item) => {
                        const itemDetails = item.details;
                        // Формируем объект фильма в формате MovieCard
                        const movieData = {
                          id: itemDetails.id || itemDetails.ident,
